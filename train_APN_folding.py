...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 25 18:08:49 2020
...
apn_exp_name = args_opt.exp_apn + '_seed' + str(random_seed)
...
apn_config['model_opt']['random_seeds'].append(random_seed)
apn_config['model_opt']['exp_name'] = apn_exp_name
apn_config['model_opt']['exp_dir'] = apn_directory # the place where logs, models, and other stuff will be stored
...
if args_opt.generate_new_splits:
    print('\n *- Generating new APN splits for random seed ', random_seed)
    ...
    action_data.generate_splits_with_seed(original_dataset_name, 
                                          path_to_original_dataset, random_seed)
    
if args_opt.generate_apn_data:
    print('\n *- Generating APN data....')
    vae_name = apn_config['model_opt']['vae_name']
    action_data.generate_latent_normalised_with_seed(vae_name, args_opt.exp_apn, 
                                                     'train', random_seed)
    action_data.generate_latent_normalised_with_seed(vae_name, args_opt.exp_apn, 
                                                     'test', random_seed)

# Initialise APN model
algorithm = getattr(alg, apn_config['algorithm_type'])(apn_config['model_opt'])
algorithm.load_norm_param_d(apn_config['data_original_opt']['path_to_norm_param_d'], 
                            random_seed)
...
if args_opt.eval_apn:
    path_to_result_file = apn_config['data_valid_opt']['path_to_result_file'] 
